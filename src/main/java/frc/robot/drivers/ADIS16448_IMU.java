...
public class ADIS16448_IMU extends GyroBase implements Gyro {
  ...
  public ADIS16448_IMU() {
    this(IMUAxis.kZ, SPI.Port.kMXP, 4, false, 18, 10);
  }

  /**
   *
   */
  public ADIS16448_IMU(final IMUAxis yaw_axis, SPI.Port port, int cal_time, boolean enableFilter, int ResetPin, int InterruptPin) {
    m_yaw_axis = yaw_axis;
    m_spi_port = port;
    ...
    //Berg - 2/26/2020 - skipping reset process because we don't have any DIO on the SPI port
    // Force the IMU reset pin to toggle on startup (doesn't require DS enable)
    m_reset_out = new DigitalOutput(ResetPin); // Drive MXP DIO8 low
    Timer.delay(0.01); // Wait 10ms
    m_reset_out.close();
    m_reset_in = new DigitalInput(ResetPin); // Set MXP DIO8 high
    ...
    if(enableFilter)
    {
      //default value is 0x0402 - see SENS_AVG on the datasheet for more info
    }
    else
    {
    ...
    //Berg - Disabling digital outputs since we're not using the main port
  ...
  boolean switchToAutoSPI() {
    ...
    if (m_auto_interrupt == null) {
      m_auto_interrupt = new DigitalInput(interrupt_pin); //MXP DIO0
    }
